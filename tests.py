...
class TicTacGameTests(unittest.TestCase):
    ...
    def test_check_winner_3(self):
        self.assertEqual(TicTacGame([1, 2, 3, 4, 'O', 'O', 'X', 'X', 'X']).check_winner(), 1)

    def test_check_winner_4(self):
        self.assertEqual(TicTacGame(['X', 2, 'O', 4, 'X', 'O', 7, 8, 'X']).check_winner(), 1)

    def test_check_winner_5(self):
        self.assertEqual(TicTacGame(['O', 'O', 'X', 4, 'X', 6, 'X', 8, 9]).check_winner(), 1)

    def test_check_winner_6(self):
        self.assertEqual(TicTacGame(['O', 'O', 'O', 4, 'X', 6, 'X', 8, 9]).check_winner(), 2)

    def test_check_winner_7(self):
        self.assertEqual(TicTacGame(['X', 2, 'X', 'O', 'O', 'O', 'O', 8, 'X']).check_winner(), 2)

    def test_check_winner_8(self):
        self.assertEqual(TicTacGame(['X', 2, 'X', 4, 'X', 6, 'O', 'O', 'O']).check_winner(), 2)

    def test_check_winner_9(self):
        self.assertEqual(TicTacGame(['O', 2, 'X', 4, 'O', 'X', 'X', 8, 'O']).check_winner(), 2)

    def test_check_winner_10(self):
        self.assertEqual(TicTacGame(['X', 'X', 'O', 'X', 'O', 6, 'O', 8, 9]).check_winner(), 2)
